...
/// # Errors
///
/// Returns `None` if `bundle`'s flags cannot be encoded in `format` (cross-address
/// transfers disabled under [`BundleFormat::PreNu6_3`]); such a bundle cannot appear
...
pub(crate) fn hash_bundle_txid_data<A: Authorization, V: Copy + Into<i64>>(
    ...
    h.update(&[bundle.flags().to_byte(format)?]);
