...
impl super::Bundle {
    ...
    /// bundle should call this before signing. A proof created with a circuit version
    /// that supports the restriction also enforces it; however, no circuit version in
    /// this crate currently does (Ironwood support is provisional), so for now the
    /// restriction is only enforced structurally: by this method, and by the
    /// equivalent checks in [`Bundle::finalize_io`] and `Bundle::create_proof`.
    ///
