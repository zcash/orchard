...
pub enum IoFinalizerError {
    /// The bundle does not satisfy the `disableCrossAddress` restriction, or is
    /// missing the `recipient` fields needed to check it.
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            IoFinalizerError::CrossAddressRestriction(e) => {
                write!(
                    f,
                    "The bundle does not satisfy the disableCrossAddress restriction: {e}"
                )
            }
