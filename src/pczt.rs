...
mod tests {
    ...
    /// Proves the preverified signing parse
    /// ([`super::Spend::parse_preverified_for_signing`] /
    /// [`super::Action::parse_preverified_for_signing`]) produces a byte-identical
    /// `spend_auth_sig` to the full parse ([`super::Spend::parse`] /
    /// [`super::Action::parse`]).
    ///
    ...
    /// and once with the preverified signing parse (which skips it).
    ...
    fn preverified_parse_signs_identically_to_full_parse() {
        ...
        // The preverified signing parse is more permissive than the full parse in exactly one
        // way: it does not validate the on-wire `fvk` bytes. A malformed `fvk` that the full
        // parse rejects with `ParseError::InvalidFullViewingKey` still parses here (as
        // `fvk: None`); the preceding full verifier pass over the identical bytes is what
        // rejects such a PCZT.
